...
if(isset($_SESSION['valid_instructor'])) {
	...
	$result_a = current_information($link, $iid);
	...
	if(mysqli_num_rows($result_a) > 0) {
		...
		echo "<table border = '1'>";
		echo "<tr>";
		echo "<td>Instructor ID: " . $row_a["iid"] . "</td>";
		...
		echo "<td>Department: " . $row_a["department"] . "</td>";
		echo "</tr>";
		echo "</table>";
		...
		if(isset($_POST["password"]) || isset($_POST["f_name"]) ||
		   ...
		   isset($_POST["department"])) {
			...
			$html_field_array = array("password", "f_name", "l_name", "email", "department");
			...
			echo "
				...
				<div>
					<form action = 'Instructor_Modify_Account.php' method = 'post'>
